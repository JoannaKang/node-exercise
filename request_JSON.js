  ...
  if (request.url === "/items") {
    if (request.method === "GET") {
      ...
      request
        ...
        .on("end", () => {
          // 클라이언트로부터 전달된 버퍼 형태의 요청을 객체로 변환합니다.
          const bodyStr = JSON.parse(Buffer.concat(body));
          // 저장된 데이터를 불러옵니다.
          const data = loadData();
          data.items.push(bodyStr);
          saveData(data);
          // 서버에서의 처리가 성공적으로 완료되었음을 알려주기 위해 응답 코드를 리턴하고 응답을 종료합니다.
